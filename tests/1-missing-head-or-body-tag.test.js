...
import fn from '../plugins/missing-head-or-body-tag';

test("full page",() => {
    let html = fs.readFileSync("samplepages/1-page-with-head-and-body.html");
    let page = new JSDOM(html);
    expect(fn(page)).toStrictEqual(["",0]);
});

test("missing head",() => {
    let html = fs.readFileSync("samplepages/1-page-without-head.html");
    let page = new JSDOM(html);
    expect(fn(page)).toStrictEqual(["Error: missing/empty <head> element",1]);
});

test("missing body",() => {
    let html = fs.readFileSync("samplepages/1-page-without-body.html");
    let page = new JSDOM(html);
